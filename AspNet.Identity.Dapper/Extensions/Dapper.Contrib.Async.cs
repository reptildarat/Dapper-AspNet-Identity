...
namespace Dapper.Contrib.Extensions
{
    ...
    public static partial class SqlMapperExtensions
    {
            ...
            int? commandTimeout = null, ISqlAdapter sqlAdapter = null) where T : class
        {
            ...
            // changes
            // assumption that if the key is not int, then its not have auto increment...so we need to include that into insert..
            if (keyProperties.Count == 1 && keyProperties[0].PropertyType != typeof(int)) keyProperties = new List<PropertyInfo>();
...
public partial class SqlServerAdapter
{
    public async Task<int> InsertAsync(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, String tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        ...
        var id = multi.Read().First().id as int? ?? 0;
